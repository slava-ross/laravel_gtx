        ...
        <div class="col-12">
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <div class="card-author"><b>Автор:</b> {{ $user->fio }}</div>
                    ...
                    <div class="card-btn">
                        ...
                        {{--@auth
                            @if (Auth::user()->id == $comment->user_id)--}}
                                <a href="{{ route('comment.edit', ['id'=>$comment->id]) }}" class="btn btn-outline-success">Редактировать</a>
                                <form action="{{ route('comment.destroy', ['id'=>$comment->id]) }}" method="post" onsubmit="if (confirm('Точно удалить отзыв?')) { return true } else { return false }">
                                    @csrf
                                    @method('DELETE')
